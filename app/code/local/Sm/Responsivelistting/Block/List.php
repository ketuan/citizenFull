...
class Sm_Responsivelistting_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function __construct($attributes = array()){
		parent::__construct();
		$this->_config = Mage::helper('responsivelistting/data')->get($attributes);
	}

	public function getConfig($name=null, $value=null){
		if (is_null($this->_config)){
			$this->_config = Mage::helper('responsivelistting/data')->get(null);
		}
	...
	protected function _toHtml(){
	 ...
	 //Zend_debug::dump($is_ajax);
		...
		if($is_ajax){
			...
			if ( $module_id = $id ){
				$template_file = 'sm/responsivelistting/default_items.phtml';
			}
			//echo json_encode($result);
		}else{
			$template_file = 'sm/responsivelistting/default.phtml';
		}	 
	...
	protected function _getItems(){
		...
		//$collect_items = array();		
		$this->addReviewSummaryTemplate('sm', 'sm/responsivelistting/summary.phtml');
		$items = array();
		foreach( $products as $k => $product ) { 
			...
			if (($maxchars=$this->getConfig('item_title_max_characs',-1))>0){
				$product_obj->title = Mage::helper('responsivelistting/data')->truncate($product_obj->full_title, $maxchars, '');
			} else {
			...
			if (($maxchars=$this->getConfig('item_desc_max_characs',-1))>0){
				$product_obj->description = Mage::helper('responsivelistting/data')->truncate($product_obj->description, $maxchars, '');
			}
	...
	protected function _getCategory(){
		...
		foreach ($category_collection as $category) {
			...
			if (($maxchars=$this->getConfig('tal_max_characters',-1))>0){
				$category_obj->title = Mage::helper('responsivelistting/data')->truncate($category_obj->full_title, $maxchars, '');
			} else {
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('responsivelistting_cfg/advanced/include_jquery')){
				...
				$import_str .= $jsHelper->includeSkinScript('sm/responsivelistting/js/jquery-1.8.2.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('sm/responsivelistting/js/jquery-noconflict.js');
