...
class Sm_Slickslider_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function __construct($attributes = array()){
		parent::__construct();
		$this->_config = Mage::helper('slickslider/data')->get($attributes);
	}

	public function getConfig($name=null, $value=null){
		if (is_null($this->_config)){
			$this->_config = Mage::helper('slickslider/data')->get(null);
		}
	...
	protected function _toHtml(){
		...
		//$template_file = "sm/slickslider/block_template_default.phtml";
		$template_file = 'sm/slickslider/'. $this->_config["theme"].'.phtml';
	...
	protected function _getMedia(){
		$items = Mage::getStoreConfig('slickslider_cfg/product_selection/product_additem');	//kq:  string(85) "a:1:{s:18:"_1289468104191_191";a:2:{s:6:"regexp";s:4:"6:20";s:5:"value";s:4:"8:20";}}" 
		if($items){
			...
			foreach($array_items as $key=>$item){
				...
				$item['sub_title'] =  Mage::helper('core/string')->truncate( $item['title']  , $this->_config['product_title_max_characters'], '...');
				if($this->_config['product_image_disp']){
				...
				$item['sub_content'] = Mage::helper('core/string')->truncate( $item['content'] , $this->_config['product_description_max_characters'],'...');		
				$item['price'] = '';
	...
	protected function _getItems(){
		...
		foreach( $products as $k => $_product ) {
			...
			$item['sub_title'] =  Mage::helper('core/string')->truncate( $_product->getName() , $this->_config['product_title_max_characters'], '...');
			$item['img'] = 'catalog/product'.$_product->getImage();
			$item['link'] = $_product->getProductUrl();
			if($this->_config['product_image_disp']){
				if($_product->getImage()){
					$item['thumb'] = (string)Mage::helper('catalog/image')->init($_product, 'image')->resize($this->getConfig('product_image_width'), $this->getConfig('product_image_height'));
				}
				else{
					$item['thumb'] = $this->getSkinUrl('sm/slickslider/images/no_image.png');
				}	
			}else{
				$item['thumb'] ='';
			}
				
			$item['cdate'] =  $_product->getCreatedAt();			
			$item['content'] = $_product->getShortDescription();
 			$item['sub_content'] = Mage::helper('core/string')->truncate( $item['content'] , $this->_config['product_description_max_characters'],'...');
			$item['price'] = $this->getPriceHtml($_product, true);//Mage::helper('core')->currency($_product->getPrice(), true, false);
			$collect_items[] = $item;
		}	
		return $collect_items;
	}
	public function getProducts(){
		...
		$this->_config['count_product'] = count($products);
        	$this->_config['totalWidthbutton1'] = ($this->_config['count_product']*18)+58;
		$this->_config['totalWidthbutton2'] = ($this->_config['count_product']*18)+58;		
		return $products;
	}	
	// public function getProducts()
	// {
		// $items = array();
		// if(is_null($this->_getProducts())) return;
		// $products = $this->_getProducts()->getItems();
		
		// foreach( $products as $k => $_product ) {
			// $tmp['id'] = $_product->getId();
			// $tmp['title'] = $_product->getName();
			// $tmp['sub_title'] = Mage::helper('core/string')->truncate( $_product->getName() , $this->_config['product_title_max_characters'],'...');
			// $tmp['img'] = 'catalog/product'.$_product->getImage();
			// $tmp['link'] = $_product->getProductUrl();
			// if($this->_config['product_image_disp']){
				...
				// $tmp['small_thumb'] =  (string)Mage::helper('catalog/image')->init($_product, 'image')->resize($this->getConfig('small_thumb_width'), $this->getConfig('small_thumb_height'));
			// }
			// else{
				// $tmp['thumb'] = '';  
				// $tmp['small_thumb'] = '';  
			// }
			// $tmp['content'] = nl2br($_product->getDescription());
			...
			// //echo $tmp['large_thumb'];
			// $items[] = $tmp;
		// }
                
		// $this->_config['count_product'] = count($items);
        	// $this->_config['totalWidthbutton1'] = ($this->_config['count_product']*18)+58;
		// $this->_config['totalWidthbutton2'] = ($this->_config['count_product']*18)+58;
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('slickslider_cfg/advanced/include_jquery')){
				// if module allowed jquery.
				$import_str .= $jsHelper->includeSkinScript('sm/slickslider/js/jquery-1.5.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('sm/slickslider/js/jsmart.noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		
		if (null == Mage::registry('jsmart.slickslider.js')){
			// add script for this module.
			$import_str .= $jsHelper->includeSkinScript('sm/slickslider/js/script.slickslider.js');
			
			Mage::register('jsmart.slickslider.js', 1);
		}
		return $import_str;
	}
	...
	private function _getResizedImage($Obj, $width, $height, $quality = 100) {
		...
		if(file_exists($imageResized)){
			...
			return $this->getSkinUrl('sm/slickslider/images/no_image.gif');
		}
	}		
}
