...
class Sm_Slider_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function __construct($attributes = array()){
        ...
        if($this->_config['control'] == 1){
            $this->_config['heightButton'] = 50;
        }else{
            $this->_config['heightButton'] = 20;
        }
        if($this->_config['control'] == 2 || $this->_config['control'] == 0){
            $this->_config['displayButton'] = 'display:none';
        }else{
			$this->_config['displayButton'] = 'display:block';
		}
        if($this->_config['autoplay'] == 1){
            $this->_config['autoplay'] = $this->_config['interval'];
        }
        $this->_config['width_module_article'] = (($this->_config['product_image_width'] + 24) * $this->_config['item_per_page']);
        $this->_config['height_module_article'] = ($this->_config['product_image_height'] + 24) * $this->_config['item_per_page'];		
	}
	...
	protected function _toHtml(){
		...
		//$template_file = "sm/slider/block_template_default.phtml";
		//$this->setTemplate($template_file);
		$template_file = 'sm/slider/'. $this->_config["theme"].'.phtml';
		$this->setTemplate($template_file);		
	...
	protected function _getMedia(){
		$items = Mage::getStoreConfig('slider_cfg/product_selection/product_additem');	//kq:  string(85) "a:1:{s:18:"_1289468104191_191";a:2:{s:6:"regexp";s:4:"6:20";s:5:"value";s:4:"8:20";}}" 
		if($items){
	...
	protected function _getItems(){
		...
		foreach( $products as $k => $_product ) {
			...
			if($_product->getImage()){
				...
				$item['thumb'] = $this->getSkinUrl('sm/slider/images/no_image.png');
			}	
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('slider_cfg/advanced/include_jquery')){
				...
				$import_str .= $jsHelper->includeSkinScript('sm/slider/js/jquery-1.5.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('sm/slider/js/jsmart.noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		
		if (null == Mage::registry('jsmart.slider.js')){
	 // add script for this module.
		 	$import_str .= $jsHelper->includeSkinScript('sm/slider/js/ymslider.js');
		 	$import_str .= $jsHelper->includeSkinScript('sm/slider/js/jquery.tooltip.js');
		 	$import_str .= $jsHelper->includeSkinScript('sm/slider/js/stickytooltip.js');
