...
// 			Zend_Debug::dump($this->_allActivedId);die;
		};
	...
	protected function _toHtml(){
		...
		//$template_file = "sm/megamenu/block_template_default.phtml";
		// $template_file = 'sm/megamenu/'. $this->_config["theme"].'.phtml';		
	...
	public function getItems(){
		...
		// $root = $helper ->getRootByGroupId($this->_config['group_id']);
		// $collection_items = $helper ->getChildsDirectlyByItem($root);
...
// 		$activedClassName = ($this->isActived($item) AND $this->hasActivedType($item))?$prefix.'actived':'';
		...
		$html = '<div class="'.$divClassName.' '.$firstClassName.' '.$prefix.'id'.$item->getId().' '.$align_right.' '.$activedClassName.' '.$item->getCustomClass().'">';
		$link = ($this->hasLinkType($item))?$this->getLinkOfType($item):'#';
		$title = ($item->getShowTitle()==Sm_Megamenu_Model_System_Config_Source_Status::STATUS_ENABLED)?'<span class="'.$prefix.'title_lv'.$item->getDepth().'">'.$item->getTitle().'</span>':'&nbsp;';
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('megamenu_cfg/advanced/include_jquery')){
				...
				$import_str .= $jsHelper->includeSkinScript('sm/megamenu/js/jquery-1.5.min.js');
