...
class EcommerceTeam_EasyCheckout_Model_GeoIP_Core
{
    ...
    static function getInstance($filename, $flags) {
        if(is_null(self::$gi)){
            ...
            $gi = new EcommerceTeam_EasyCheckout_Model_GeoIP_Core;
            $gi->flags = $flags;
            if ($gi->flags & self::GEOIP_SHARED_MEMORY) {
    ...
    public function geoip_record_by_addr($addr){
        return EcommerceTeam_EasyCheckout_Model_GeoIP_City::geoip_record_by_addr($this,$addr, $this);
    }

    public function geoip_country_code_by_addr($addr) {
        $gi = $this;
      if ($gi->databaseType == self::GEOIP_CITY_EDITION_REV1) {
        $record = EcommerceTeam_EasyCheckout_Model_GeoIP_City::geoip_record_by_addr($gi,$addr, $this);
        if ( $record ) {
