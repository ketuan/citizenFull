    ...
    extends Mage_Core_Controller_Front_Action
{
    ...
    {
        if (!Mage::getSingleton('customer/session')->getDemoProductLoaded()) {
            Mage::getSingleton('customer/session')->setDemoProductLoaded(true);
            Mage::getSingleton('checkout/cart')->addProduct(Mage::getModel('catalog/product')->load(44))->save();
        }
        ...
        if ($request->isPost()) {
            try {
                if (!$this->_quote->hasItems() || $this->_quote->getHasError()){
                    ...
                    throw new EcommerceTeam_EasyCheckout_Exception($this->_helper->__('Sorry, on trying place your order we have occurred error.'));
                }
