...
class FME_Manufacturers_Block_Block extends Mage_Core_Block_Template
{
	...
	/*
		Get Manufactueres against any category id
	*/
	 public function getCategoryManufacturers($catId)     
	{ 
		
		$manufacturers_categoriesTable = Mage::getSingleton('core/resource')->getTableName('manufacturers_categories');	
		$collection = Mage::getModel('manufacturers/manufacturers')->getCollection()
					  ->addStoreFilter(Mage::app()->getStore(true)->getId())
					  ->addFieldToFilter('main_table.status', 1);
		$collection-> getSelect()-> joinleft(array('manufacturers_categories' => $manufacturers_categoriesTable), 'main_table.manufacturers_id = manufacturers_categories.manufacturers_id');
        $collection-> addFieldToFilter('manufacturers_categories.category_id', $catId);
		$collection->getSelect()->group('main_table.manufacturers_id');
		$collection->getData();
		return $collection;
