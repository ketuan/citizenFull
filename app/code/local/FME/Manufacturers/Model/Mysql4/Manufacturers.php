...
class FME_Manufacturers_Model_Mysql4_Manufacturers extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
		...
		if(isset($_POST['new_categories_ids'])) {
			$string = $_POST['new_categories_ids'];
			$string = trim($string, ',');
			//echo "<pre>";print_r($_POST);exit;
			if(isset($_POST['new_categories_ids'])) {
					$manufacturersCatsTable = Mage::getSingleton('core/resource')->getTableName('manufacturers_categories');
					//echo "DELETE FROM ".$manufacturersCatsTable." WHERE category_id in ($string) and manufacturers_id=".$object->getId();exit;
					$db = $this->_getWriteAdapter();
					try {
							$db->beginTransaction();
							$db->exec("DELETE FROM ".$manufacturersCatsTable." WHERE manufacturers_id=".$object->getId());
							$db->commit();
			...
			$result = array_unique($catIds);
			 ...
			 foreach ($result as $category) {
				$categoryArray = array();
				$categoryArray['manufacturers_id'] = $object->getId();
				$categoryArray['category_id'] = $category;
				$this->_getWriteAdapter()->insert($this->getTable('manufacturers_categories'), $categoryArray);
			}
		}
